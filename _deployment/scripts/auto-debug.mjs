...
async function testAuthEndpoints(testUser) {
  ...
  const noAuth = await fetchApi('/auth/me');
  if (noAuth.status !== 401) throw new Error('Should return 401 without token');
  
  // Test with token
  const withAuth = await fetchApi('/auth/me', {
    headers: { Authorization: `Bearer ${testUser.token}` }
  });
  if (!withAuth.ok) throw new Error(`GET /auth/me: ${withAuth.data.message || 'Failed'}`);
}
...
async function testProfileEndpoints(testUser) {
  // GET profile
  const getProfile = await fetchApi('/profile', {
    headers: { Authorization: `Bearer ${testUser.token}` }
  });
  if (!getProfile.ok) throw new Error(`GET /profile: ${getProfile.data.message || 'Failed'}`);
  
  // PUT profile (just update with same data)
  const updateProfile = await fetchApi('/profile', {
    method: 'PUT',
    headers: { Authorization: `Bearer ${testUser.token}` },
...
async function testRedisConnection() {
  // Try import redis config
  try {
    const response = await fetch(`${API_BASE.replace('/api', '')}/health`);
    // If server starts, Redis is at least configured (may be mocked)
    if (!response.ok) throw new Error('Health check failed');
  } catch (e) {
    throw new Error(`Redis/Server issue: ${e.message}`);
  }
}
...
async function main() {
  ...
  for (let i = 0; i < 10; i++) {
    try {
      const res = await fetch(`${API_BASE.replace('/api', '')}/health`, { timeout: 2000 });
      if (res.ok) {
        serverReady = true;
        break;
      }
  ...
  await test('Redis/Server Integration', testRedisConnection);
